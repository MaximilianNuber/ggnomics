...
```{python}
#| label: readme-baron-composition
...
```{python}
#| label: readme-significance-brackets
...
# Marsilea renders through matplotlib but does not return a figure Quarto can
# capture automatically, so the README copy is written out explicitly. This
# is the only one of the README images not captured via Quarto's normal
# figure-html output.
readme_img_dir = pathlib.Path("..") / "docs" / "img" / "readme"
readme_img_dir.mkdir(parents=True, exist_ok=True)
h.save(str(readme_img_dir / "marsilea_heatmap.png"))
```

![Marsilea heatmap of the top 25 significant genes, 24h salt vs. control](../docs/img/readme/marsilea_heatmap.png)
...
```{python}
#| label: readme-tcr-clonotype-abundance
