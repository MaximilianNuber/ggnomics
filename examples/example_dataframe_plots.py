...
p6 = heatmap_from_matrix(
    mat, row_names=[f"g{i}" for i in range(10)], col_names=[f"c{j}" for j in range(5)], zscore_rows=True
)
print(p6)
...
# 7) Composition
p8 = cluster_composition_barplot(
    df, cluster_col="cluster", group_col="condition", normalize=True, title="Cluster composition"
)
print(p8)
