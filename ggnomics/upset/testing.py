...
from .intersections import select_mode_observations, upset_data
from .modes import Mode, normalize_mode

_OPTIONAL_DEPENDENCY_MESSAGE = (
    '{module} is required for ggnomics.upset statistical helpers. Install with: pip install "ggnomics[upset]"'
)
...
def _scipy_stats():
    try:
        ...
        raise ImportError(_OPTIONAL_DEPENDENCY_MESSAGE.format(module="scipy")) from exc
    return stats
...
def _multipletests():
    try:
        ...
        raise ImportError(_OPTIONAL_DEPENDENCY_MESSAGE.format(module="statsmodels")) from exc
...
def _extract_test_result(result: Any) -> tuple[float, float, str]:
    ...
    if statistic is None or p_value is None:
        if isinstance(result, tuple) and len(result) >= 2:
            ...
            raise TypeError(
                "test must return an object with statistic and pvalue attributes or a (statistic, p_value) tuple"
            )
...
def _default_variable_test(
    ...
    if pd.api.types.is_numeric_dtype(values.dtype):
        groups = [part[variable].dropna().to_numpy() for _, part in frame.groupby(group_column, observed=True)]
    ...
    contingency = pd.crosstab(frame[group_column].to_numpy(), values.to_numpy(), dropna=True)
...
def compute_intersection_tests(
    ...
    *,
    ...
    if canonical_mode != "exclusive_intersection":
        warnings.warn(
            "Non-exclusive modes repeat observations across intersections; test groups are therefore not independent.",
            UserWarning,
            stacklevel=2,
        )
    frame = select_mode_observations(data, mode=canonical_mode)
    ...
    if ignore_mode_columns:
        ignored.update(column for column in frame.columns if column.startswith("in_") or column.endswith("_size"))
    ...
    for variable in variables:
        ...
        if selected_test is None:
            statistic, p_value, method = _default_variable_test(frame, variable, "intersection")
        else:
            groups = [part[variable].dropna().to_numpy() for _, part in frame.groupby("intersection", observed=True)]
            ...
            if len(groups) < 2:
                ...
                statistic, p_value, method = _extract_test_result(selected_test(*groups))
        rows.append(
            {
...
def adjust_intersection_tests(
    ...
    *,
    ...
    adjusted["fdr"] = np.nan
    if valid.any():
        adjusted.loc[valid, "fdr"] = _multipletests()(adjusted.loc[valid, "p_value"].to_numpy(), method=method)[1]
    return adjusted
...
def upset_test(
    ...
    **upset_data_kwargs: Any,
    ...
    return (
        adjust_intersection_tests(results, method=correction)
        .sort_values("fdr", na_position="last", kind="stable")
